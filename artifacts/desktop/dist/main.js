...
electron_1.app.commandLine.appendSwitch("enable-webgpu");
electron_1.app.commandLine.appendSwitch("use-angle", "d3d11");
electron_1.app.commandLine.appendSwitch("enable-features", "VaapiVideoDecoder,VaapiVideoEncoder");
...
function startApiServer() {
    return new Promise((resolve) => {
        if (IS_DEV) {
            ...
            apiProcess = (0, child_process_1.spawn)(cmd, args, { env, stdio: ["ignore", "pipe", "pipe"], detached: false, windowsHide: true });
            const timeout = setTimeout(() => { if (!apiReady) {
            ...
            apiProcess.stdout?.on("data", (data) => {
                ...
                if (msg.includes("listening") || msg.includes(String(API_PORT))) {
                ...
                const checkReady = (attempt) => {
                    ...
                    req.on("error", () => {
                        if (attempt < 20)
                            ...
                            console.error("[API] Server never became ready");
                            resolve();
                        }
                    });
...
// --- Crash Logger -------------------------------------------------------------
function _logCrash(detail) {
    try {
        const logDir = appIsReady ? electron_1.app.getPath("logs") : os.tmpdir();
...
function writeDiagnosticLog() {
    try {
        ...
        const userDataPath = appIsReady ? electron_1.app.getPath("userData") : "not-ready";
        const logsPath = appIsReady ? electron_1.app.getPath("logs") : "not-ready";
...
function updateSplash(pct, text) {
    if (!splashWindow || splashWindow.isDestroyed())
        return;
    splashWindow.webContents.executeJavaScript(`window.__updateSplash?.(${pct}, ${JSON.stringify(text)})`).catch(() => { });
}
...
async function createMainWindow() {
    ...
    let bounds = { width: 1440, height: 900, x: undefined, y: undefined };
    try {
        ...
        for (let i = 0; i < maxRetries && !loaded; i++) {
            try {
                ...
                if (ok) {
                    ...
                    const versionFile = path.join(electron_1.app.getPath("userData"), "last-version.txt");
    ...
    const contextMenu = electron_1.Menu.buildFromTemplate([
        ...
        },
        {
            ...
            click: () => {
                mainWindow?.show();
                mainWindow?.focus();
                mainWindow?.webContents.executeJavaScript(`window.__saisai-rolotech-smart-engines_navigate?.("roll")`);
            },
        },
        {
            ...
            click: () => {
                mainWindow?.show();
                mainWindow?.focus();
                mainWindow?.webContents.executeJavaScript(`window.__saisai-rolotech-smart-engines_navigate?.("troubleshoot")`);
            },
        },
        { type: "separator" },
        {
            label: "API Status",
            click: () => electron_1.shell.openExternal(`http://localhost:${API_PORT}/api/health`),
        },
        { type: "separator" },
        {
            label: `Quit ${APP_NAME}`,
    ...
    const template = [
            ...
            submenu: [
                {
                    ...
                    click: async () => {
                        ...
                        if (!result.canceled && result.filePaths.length > 0) {
                            ...
                            mainWindow?.webContents.executeJavaScript(`window.__saisai-rolotech-smart-engines_loadDxf?.(${JSON.stringify(content)}, ${JSON.stringify(path.basename(filePath))})`);
                        }
                    },
                },
                {
                    ...
                    click: async () => {
                        ...
                        if (!result.canceled && result.filePath) {
                            mainWindow?.webContents.executeJavaScript(`window.__saisai-rolotech-smart-engines_getGcode?.()`).then((gcode) => {
                                if (gcode)
                ...
                },
                {
                    ...
                    click: async () => {
                        ...
                        if (!result.canceled && result.filePath) {
                            mainWindow?.webContents.executeJavaScript(`JSON.stringify(JSON.parse(localStorage.getItem("sai-rolotech-smart-enginesai-cnc-v2") || "{}"), null, 2)`).then((data) => {
                                if (data)
                ...
                },
                {
                    ...
                    click: async () => {
                        ...
                        if (!result.canceled && result.filePaths.length > 0) {
                            ...
                            mainWindow?.webContents.executeJavaScript(`localStorage.setItem("sai-rolotech-smart-enginesai-cnc-v2", ${JSON.stringify(data)}); window.location.reload();`);
                        }
                    },
                },
                { type: "separator" },
            ...
            submenu: [
                {
                    ...
                    click: () => electron_1.shell.openExternal(`http://localhost:${API_PORT}/api/ai/status`),
                },
                {
                ...
                },
                {
                    ...
                    click: () => electron_1.shell.openPath(electron_1.app.getPath("logs")),
                },
                { type: "separator" },
                {
                    label: "Dark Mode",
                    type: "checkbox",
                    checked: electron_1.nativeTheme.shouldUseDarkColors,
                    click: (item) => {
                        electron_1.nativeTheme.themeSource = item.checked ? "dark" : "light";
                    },
                },
            ],
        },
        {
            ...
            submenu: [
                ...
                {
                    label: "Documentation",
                    click: () => electron_1.shell.openExternal("https://saisai-rolotech-smart-engines.com/docs"),
                },
                {
...
function setupIPC() {
    ...
    electron_1.ipcMain.handle("save-file", async (_event, { content, defaultName, filters }) => {
        const result = await electron_1.dialog.showSaveDialog(mainWindow, {
    ...
    // Open file natively
    electron_1.ipcMain.handle("open-file", async (_event, { filters }) => {
        const result = await electron_1.dialog.showOpenDialog(mainWindow, {
            filters: filters ?? [{ name: "All Files", extensions: ["*"] }],
            properties: ["openFile"],
        });
    ...
    electron_1.ipcMain.handle("get-gpu-info", async () => {
        try {
    ...
    const _nvDefault = { available: false, name: "", utilGpu: 0, memUsed: 0, memTotal: 0, tempC: 0, powerW: 0, clockMHz: 0 };
    let _hwCache = null;
    let _cpuPrev = [];
    function _snapCpus() {
    ...
    async function _refreshHW() {
        try {
            ...
            const now = Date.now();
            if (!_hwCache?._nvidiaTs || now - (_hwCache?._nvidiaTs ?? 0) > 3000) {
                try {
                    const { execFile } = await Promise.resolve().then(() => __importStar(require("child_process")));
                    nvidia = await new Promise((resolve) => {
                        execFile("nvidia-smi", ["--query-gpu=name,utilization.gpu,memory.used,memory.total,temperature.gpu,power.draw,clocks.current.graphics", "--format=csv,noheader,nounits"], { timeout: 2000 }, (err, stdout) => {
                            if (err || !stdout) {
                                resolve(_nvDefault);
                                return;
                            }
            ...
            _hwCache._nvidiaTs = nvidia._ts ?? (_hwCache._nvidiaTs ?? 0);
        }
        catch (e) {
    ...
    electron_1.app.on("before-quit", () => clearInterval(_hwTimer));
    // IPC returns cached data instantly — zero blocking
    electron_1.ipcMain.handle("get-live-hardware", () => {
        return _hwCache ?? {
            coreUtils: [], avgCpu: 0,
    ...
    electron_1.ipcMain.handle("quit-and-install", () => {
        if (global.__updateTimer) {
            clearInterval(global.__updateTimer);
            global.__updateTimer = null;
        }
        isQuitting = true;
        electron_updater_1.autoUpdater.quitAndInstall();
    });
    electron_1.ipcMain.handle("get-update-settings", () => {
        const settingsPath = path.join(electron_1.app.getPath("userData"), "update-settings.json");
        try {
    ...
    electron_1.ipcMain.handle("set-update-settings", (_event, settings) => {
        const settingsPath = path.join(electron_1.app.getPath("userData"), "update-settings.json");
        const current = (() => {
            try {
                return JSON.parse(fs.readFileSync(settingsPath, "utf8"));
    ...
    electron_1.ipcMain.handle("get-update-history", () => {
        const historyPath = path.join(electron_1.app.getPath("userData"), "update-history.json");
        try {
    ...
    electron_1.ipcMain.on("show-notification", (_event, { title, message }) => {
...
function logUpdateHistory(entry) {
    const historyPath = path.join(electron_1.app.getPath("userData"), "update-history.json");
    let history = [];
    try {
...
function getLicenseKeyPath() {
    return path.join(electron_1.app.getPath("userData"), "license-key.dat");
}
function getTrialTimestampPath() {
    return path.join(electron_1.app.getPath("userData"), ".trial-ts");
}
function getHardwareIdPath() {
    return path.join(electron_1.app.getPath("userData"), ".hw-bind");
}
function getIntegrityPath() {
    return path.join(electron_1.app.getPath("userData"), ".integrity");
}
...
function getHardwareId() {
    ...
    if (IS_WIN) {
        try {
            const serial = (0, child_process_1.execSync)('wmic baseboard get serialnumber', { encoding: "utf8", windowsHide: true });
            parts.push(serial.replace(/\s+/g, "").replace("SerialNumber", ""));
        }
        catch { /* ignore */ }
        try {
            const diskId = (0, child_process_1.execSync)('wmic diskdrive get serialnumber', { encoding: "utf8", windowsHide: true });
            parts.push(diskId.replace(/\s+/g, "").replace("SerialNumber", ""));
        }
        catch { /* ignore */ }
        try {
            const biosSerial = (0, child_process_1.execSync)('wmic bios get serialnumber', { encoding: "utf8", windowsHide: true });
...
function verifyHardwareBinding() {
    try {
...
function generateIntegrityHash() {
    ...
    const mainJsPath = IS_DEV ? __filename : path.join(electron_1.app.getAppPath(), "dist", "main.js");
    const parts = [];
    parts.push(exePath);
...
function saveLicenseEncrypted(key) {
    try {
...
function startRuntimeLicenseCheck() {
...
function getSavedLicenseKey() {
    ...
    if (IS_WIN) {
        try {
            const result = (0, child_process_1.execSync)('reg query "HKCU\\Software\\SAI Rolotech Smart Engines" /v ProductKey', { encoding: "utf8", windowsHide: true });
...
function getActivationPath() {
    return path.join(electron_1.app.getPath("userData"), "activation.dat");
}
...
async function verifyTokenOnline(token, hwId) {
    try {
        const http = require("http");
...
function getSystemInfoForRegistration() {
    const cpus = os.cpus();
    return {
        hostname: os.hostname(),
        platform: os.platform(),
...
async function showRegistrationForm(prefilledKey) {
    ...
    return new Promise((resolve) => {
        ...
        win.on("page-title-updated", (_e, title) => {
            if (title.startsWith("REG:")) {
                try {
                    const data = JSON.parse(title.slice(4));
                    resolved = true;
                    win.close();
                    resolve(data);
                }
...
async function verifyLicense() {
    ...
    while (attempts < maxAttempts) {
        ...
        const loadingWin = new electron_1.BrowserWindow({
        ...
        loadingWin.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(`<html><body style="margin:0;background:#0a0a1a;display:flex;align-items:center;justify-content:center;height:100vh;font-family:'Segoe UI',sans-serif;color:#f97316;font-size:16px;font-weight:600;">
...
electron_1.app.whenReady().then(async () => {
    ...
    if (!IS_DEV) {
        setupAutoUpdater();
        setTimeout(() => electron_updater_1.autoUpdater.checkForUpdatesAndNotify(), 10000);
    }
    try {
        electron_1.globalShortcut.register("Ctrl+Shift+R", () => mainWindow?.webContents.reload());
    }
    catch (e) {
...
process.on("uncaughtException", (error) => {
    ...
    if (appIsReady && mainWindow) {
        try {
            electron_1.dialog.showErrorBox("SAI Rolotech  -  Error Recovered", `An unexpected error occurred but was caught safely.\n\nError: ${error.message}\n\nThe application will continue. If problems persist, use File -> Restart.`);
        }
