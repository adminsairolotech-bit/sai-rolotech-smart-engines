/**
 * SEMI_UPPER_ROLL - Roll Forming Semi Agent
 * ===========================================
 * Upper roll design agent
 */
...
export const CONFIG: SemiAgentConfig = {
  name: 'SEMI_UPPER_ROLL',
  version: '1.0.0',
  timeout: 15000,
  retries: 2,
};

export interface UpperRollInput {
  centerline: Point2D[];
  thickness: number;
  formingAngle: number;
  material: string;
...
export interface UpperRollOutput {
  roll: Roll;
  groove: GrooveGeometry;
...
export async function designUpperRoll(
  input: UpperRollInput,
  context: SemiAgentContext
): Promise<SemiAgentResult<UpperRollOutput>> {
  try {
    const offset = input.offset || (input.thickness / 2 + 3);
    const profile = generateUpperProfileContour(input.centerline, offset);

    const rollDiameter = input.rollDiameter || calculateRollDiameter(input.centerline, input.thickness);
    ...
    const roll: Roll = {
      id: `upper_roll_${context.projectId}`,
      profile: rollProfile,
    ...
    const contactPoints = calculateContactPoints(profile, rollDiameter);
    ...
    return {
      success: false,
      error: `Upper roll design failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
    };
  }
}

function generateUpperProfileContour(centerline: Point2D[], offset: number): Point2D[] {
...
function calculateGrooveGeometry(profile: Point2D[], thickness: number): GrooveGeometry {
...
function convertToSegments(contour: Point2D[]): RollProfileSegment[] {
  ...
  for (let i = 0; i < contour.length - 1; i++) {
    ...
    segments.push({
      type: i < contour.length / 2 ? 'flange' : 'web',
      startX: p1.x,
      startY: p1.y,
      endX: p2.x,
      endY: p2.y,
...
function calculateContactPoints(profile: Point2D[], rollDiameter: number): Point2D[] {
  const rollRadius = rollDiameter / 2;
  const contactPoints: Point2D[] = [];

  for (let i = 0; i < profile.length; i += Math.floor(profile.length / 8)) {
    const point = profile[i];
    const dist = Math.sqrt(point.x * point.x + point.y * point.y);

    if (dist > 0) {
      contactPoints.push({
        x: (point.x / dist) * rollRadius,
        y: (point.y / dist) * rollRadius,
      });
    }
  }
...
export function validateUpperRoll(roll: Roll, thickness: number): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export const SemiUpperRoll = {
  config: CONFIG,
  designUpperRoll,
  validateUpperRoll,
};

export default SemiUpperRoll;
